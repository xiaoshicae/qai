use tauri::State;

use crate::db::init::DbState;
use crate::errors::AppError;
use crate::models::collection::{Collection, CollectionTreeNode};
use crate::models::group::Group;

#[tauri::command]
pub fn list_collections(db: State<'_, DbState>) -> Result<Vec<Collection>, AppError> {
    let conn = db.conn()?;
    let result = crate::db::collection::list_all(&conn);
    match &result {
        Ok(cols) => log::info!("list_collections: {} collections loaded", cols.len()),
        Err(e) => log::error!("list_collections failed: {}", e),
    }
    Ok(result?)
}

#[tauri::command]
pub fn create_collection(
    db: State<'_, DbState>,
    name: String,
    description: String,
    group_id: Option<String>,
) -> Result<Collection, AppError> {
    let conn = db.conn()?;
    Ok(crate::db::collection::create(
        &conn,
        &name,
        &description,
        group_id.as_deref(),
    )?)
}

#[tauri::command]
pub fn update_collection(
    db: State<'_, DbState>,
    id: String,
    name: Option<String>,
    description: Option<String>,
    group_id: Option<Option<String>>,
    sort_order: Option<i32>,
) -> Result<Collection, AppError> {
    let conn = db.conn()?;
    let gid = group_id.map(|outer| outer.as_deref().map(|s| s.to_string()));
    let gid_ref = gid.as_ref().map(|o| o.as_deref());
    Ok(crate::db::collection::update(
        &conn,
        &id,
        name.as_deref(),
        description.as_deref(),
        gid_ref,
        sort_order,
    )?)
}

#[tauri::command]
pub fn delete_collection(db: State<'_, DbState>, id: String) -> Result<(), AppError> {
    let conn = db.conn()?;
    Ok(crate::db::collection::delete(&conn, &id)?)
}

#[tauri::command]
pub fn get_collection_tree(
    db: State<'_, DbState>,
    collection_id: String,
) -> Result<CollectionTreeNode, AppError> {
    let conn = db.conn()?;
    Ok(crate::db::collection::get_tree(&conn, &collection_id)?)
}

// --- Group 命令 ---

#[tauri::command]
pub fn list_groups(db: State<'_, DbState>) -> Result<Vec<Group>, AppError> {
    let conn = db.conn()?;
    Ok(crate::db::group::list_all(&conn)?)
}

#[tauri::command]
pub fn create_group(
    db: State<'_, DbState>,
    name: String,
    parent_id: Option<String>,
) -> Result<Group, AppError> {
    let conn = db.conn()?;
    Ok(crate::db::group::create(
        &conn,
        &name,
        parent_id.as_deref(),
    )?)
}

#[tauri::command]
pub fn update_group(
    db: State<'_, DbState>,
    id: String,
    name: Option<String>,
    parent_id: Option<Option<String>>,
    sort_order: Option<i32>,
) -> Result<Group, AppError> {
    let conn = db.conn()?;
    let pid = parent_id.map(|outer| outer.as_deref().map(|s| s.to_string()));
    let pid_ref = pid.as_ref().map(|o| o.as_deref());
    Ok(crate::db::group::update(
        &conn,
        &id,
        name.as_deref(),
        pid_ref,
        sort_order,
    )?)
}

/// 设置分组的 parent_id（支持 null → 提升为顶级）
/// 独立命令，避免 Option<Option<String>> 的 serde null 反序列化问题
#[tauri::command]
pub fn set_group_parent(
    db: State<'_, DbState>,
    id: String,
    parent_id: Option<String>,
) -> Result<Group, AppError> {
    let conn = db.conn()?;
    conn.execute(
        "UPDATE groups SET parent_id = ?2 WHERE id = ?1",
        rusqlite::params![id, parent_id],
    )?;
    Ok(crate::db::group::get(&conn, &id)?)
}

#[tauri::command]
pub fn delete_group(db: State<'_, DbState>, id: String) -> Result<(), AppError> {
    let conn = db.conn()?;
    Ok(crate::db::group::delete(&conn, &id)?)
}

#[derive(serde::Deserialize)]
pub struct GroupOrder {
    pub id: String,
    pub sort_order: i32,
}

#[derive(serde::Deserialize)]
pub struct CollectionOrder {
    pub id: String,
    pub group_id: Option<String>,
    pub sort_order: i32,
}

#[tauri::command]
pub fn reorder_sidebar(
    db: State<'_, DbState>,
    groups: Vec<GroupOrder>,
    collections: Vec<CollectionOrder>,
) -> Result<(), AppError> {
    let conn = db.conn()?;
    for g in &groups {
        conn.execute(
            "UPDATE groups SET sort_order = ?1 WHERE id = ?2",
            rusqlite::params![g.sort_order, g.id],
        )?;
    }
    for c in &collections {
        conn.execute(
            "UPDATE collections SET sort_order = ?1, group_id = ?2 WHERE id = ?3",
            rusqlite::params![c.sort_order, c.group_id, c.id],
        )?;
    }
    Ok(())
}
